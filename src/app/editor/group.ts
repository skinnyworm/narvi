...
export function group(datasource: DataSource, label: LabelSpec, output: OutputSpec[]): GroupResult {
  const { data, meta } = datasource;
  const keys = meta.schema.map((item) => `$${item.field}`);
  const labelExp = parse(label);

  const getRow = (values: Value[]) =>
    values.reduce<Record<string, Value>>((row, value, i) => ({ ...row, [keys[i]]: value }), {});
  ...
  const state = data.reduce<Record<string, LabelState>>((resultData, values) => {
  ...
  return Object.entries(state).reduce<GroupResult>((labelRecords, [label, labelState]) => {
    const labelValues = Object.entries(labelState).reduce<LabelOutput>((labelValues, [label, expression]) => {
